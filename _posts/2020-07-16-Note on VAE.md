...
* Decoder(generator)는 오직 embedding means $\mu_{\phi}(x)$만을 결과값으로 반환한다. 이때 embedding variance는 결과값으로 반환하지 않고, 학습하지 않는 pglobal variance로 1을 설정한다.

이는 우리가 학습에 사용하는 데이터에 그 이유가 있다. 일반적으로 VAE를 실험할 때 MNIST 데이터를 주로 사용하게 된다. MNIST 데이터를 사용할 때는 보통 [-1, 1]로 pixel 값을 scaling한 뒤 사용하게 된다.

따라서, MNIST의 데이터 분포를 평균이 0인 정규분포를 이용한다면 [-1, 1] 범위의 값은 그 분산이 1인 정규분포라고 생각해도 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMzAyNjc5OTQsLTQ3NjU2NTAwNyw5Mj
