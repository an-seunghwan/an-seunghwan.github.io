...
1. 조건부 기댓값 : $Q_n(a, x) = \hat{\mathbb{E}}[Y \vert A=a, X=x]$ where $\hat{P}_n$ be any distribution of $(X, A, Y)$ such that
	- $X$의 주변 분포가 empirical distribution으로 주어지고,
	- $Y$ given $A = a$ for $a = 0, 1$ and $X = X_i$ for $i = 1, \cdots, n$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTE0MzE3NTg2OSwxODM3ODYwODIyLDIwNT
