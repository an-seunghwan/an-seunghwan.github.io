...
3. The nonparametric EIF of $P \mapsto V(f_0, P)$ at $P_0$:

$$
\phi_0: z \mapsto \frac{I(a = f_0(x))}{\pi_0(f_0(x), x)} (y - Q_0(f_0(x), x)) + Q_0(f_0(x), x) - V(f_0, P_0) 
$$

4. $V(f_n, \hat{P}_n) = \frac{1}{n}\sum_{i=1}^n \hat{\mathbb{E}}[Y \vert A=A_i, X=X_i]$
5. $V(f_{n, s}, \hat{P}_n) = \frac{1}{n}\sum_{i=1}^n Q_{n,s}(A_i, X_i)$
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMDAxMTA1MjQsMTgzNzg2MDgyMiwyMD
