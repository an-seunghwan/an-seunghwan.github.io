...
해석
	...
	- 만약 우리가 전체 설명변수에서 $X_5$를 제외한다면, 우리는 oracle potential outcome mean (predictiveness measure)
    \item For $s = \{5\}$, VIM $\psi_{n, s} = 0.64$ and the asymptotically valid 95\% confidence intervals for $\psi_{0, s}$ is 
    \item If we exclude $X_5$ from full covariates, we lost 0.64 of oracle potential outcome mean

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk3NjkyNjY0OSwtMjA4NjgxMTU4MiwtMT
