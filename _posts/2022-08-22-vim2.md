...
- 데이터 개수 $n = 1000$
- 변수 개수 $p = 5$
- treatment $A \sim Ber(\sigma())$
- treatment $$A \sim \mbox{Bernoulli}(\text{sigmoid}(0.5 \cdot X_1 -0.4 \cdot X_2 + 0.3 \cdot X_3 + 0.2 \cdot X_4 -0.1 \cdot X_5))$$
    \item Conditional ATE: $\tau(X) = 1 \cdot X_1 -2 \cdot X_2 -3 \cdot X_3 -4 \cdot X_4 +5 \cdot X_5$
    \item $Y \sim N(-5 \cdot X_1 -4 \cdot X_2 + 3 \cdot X_3 -2 \cdot X_4 +1 \cdot X_5 + A \cdot \tau(X), 1)$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc1Mzk0NzczMSwxODM3ODYwODIyLDIwNT
