...
해석
	- $X_5$가 가장 높은 population-level 중요도를 가짐
		- population-level 중요도란? 전체 설명변수에서 
    \item[-] the amount of oracle predictiveness lost by excluding $X_5$ from full covariates
    \item For $s = \{5\}$, VIM $\psi_{n, s} = 0.64$ and the asymptotically valid 95\% confidence intervals for $\psi_{0, s}$ is $(0.20, 1.08)$
    \item If we exclude $X_5$ from full covariates, we lost 0.64 of oracle potential outcome mean

<!--stackedit_data:
eyJoaXN0b3J5IjpbMzgwMzU0MTQ4LC0yMDg2ODExNTgyLC0xNz
