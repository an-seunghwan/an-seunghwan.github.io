...
def clean_josa(sent):
    ...
    return result
```
조사 전처리를 추가한 경우에 앞과 동일하게 BPE 알고리즘을 거친 후의 결과를 확인해보자.
```python
subwords_dict = vocab_segmentation(vocab)
print('subwords 사전의 개수: {}'.format(len(subwords_dict)))
pprint(subwords_dict[:50])
...
> [http://nlp.kookmin.ac.kr/data/han-dic.html](http://nlp.kookmin.ac.kr/data/han-dic.html)
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwODE3NzQwMjgsMTIzOTQ2MTM5MCw4Nj
AwNjQ4NDcsLTE1NTE2MTI4ODMsMTE5MDQ4MjMwXX0=
-->