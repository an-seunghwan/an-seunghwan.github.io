...
또한 품사 태깅 이전에 한글화를 하는 이유는 시간적인 문제도 포함된다. `Okt()`를 이용해 품사 태깅을 하는 과정이 상당히 오래 걸리는 것을 직접 코드를 돌려봤다면 금방 알 수 있다. 따라서 최대한 데이터를 정제하여 불필요한 데이터를 제거하여 품사 태깅 과정을 시작하는 것이 좋다.
...
['만', '아니다', '별', '다섯', '개', '주다', '왜', '로', '나오다', '제', '심기', '불편하다', '하다']
```
최종 정제된 후의 데이터를 보면 '3D'가 제외되어 있는 것을 볼 수 있다. 따라서 이렇게 한글 정제를 하는 경우에는 혹시 영어로 된 단어가 유의미하게 사용될 수 있는지 면밀히 확인할 필요성이 있ㄷ

추가 정제 - 불용어 제거
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTgyOTIzNzM5LDc5NzY4Nzc2NCwtMTQwMD
M3ODQ2MCw2MjMxMDgzODYsLTEyMzA1NDEwOTksLTM0Mjg4MTE0
NiwxMjEzNjAxMzAwLDcwNjYyODI3NiwtMjYyOTgzNzU5XX0=
-->