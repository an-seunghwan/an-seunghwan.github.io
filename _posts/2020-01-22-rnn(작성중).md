...
### <span style="color:#2874A6;">0. 배경</span>
...
- $\textbf{x}_t$: 
$t$th timestep input of sequence, 
shape = ($d_{\mathbf{x}}$, 1)
- $\mathbf{h}_{t}$: hidden state corresponding to $t$th timestep input, shape = ($d_{\mathbf{h}}$, 1)
- $\mathbf{W}_\mathbf{x}$: input sequence에 대한 가중치 행렬, shape = ($d_{\mathbf{h}}$, $d_{\mathbf{x}}$) 
- $\mathbf{W}_\mathbf{h}$: hidden state에 대한 가중치 행렬, shape = ($d_{\mathbf{h}}$, $d_{\mathbf{h}}$) 
- $\mathbf{W}_\mathbf{y}$: output에 대한 가중치 행렬

일반적으로 $\mathbf{x}_t$는 기계번역 분야에서 하나의 단어(token)으로 간주된다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQzOTI2NjIyNyw2MDE0MDEwNjYsMTMzNT
kwNjk0OCwtMTI3OTQzOTc0MSwtMTkyNzYzNzE1MiwtMTAzMDQ4
MzU3NF19
-->