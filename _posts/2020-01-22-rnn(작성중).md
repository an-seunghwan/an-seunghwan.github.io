...
그렇다면 `return_sequences`의 역할을 알아보자. `return_sequences`는 각 RNN cell에서 계산되는 hidden state에 대한 return 여부를 나타낸다. 즉, 앞의 예제들은 `return_sequences=False`이므로 output으로 계산된 결과는 모두 마지막 timestep에 해당하는 RNN cell의 hidden state이다. `return_sequences=True`로 하게되면 모든 RNN cell에서 hidden
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDI0ODgyNzk0LDE2MjQ1MTc1MzgsLTE4Nj
kyNjcyNzgsLTE0NTE0MTI1NjUsMTg3MzI5ODg3NywxODQ3Njky
NjUyLC0xNDgxNTQwMjQyLDYwMTQwMTA2NiwxMzM1OTA2OTQ4LC
0xMjc5NDM5NzQxLC0xOTI3NjM3MTUyLC0xMDMwNDgzNTc0XX0=
