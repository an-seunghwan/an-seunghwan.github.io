...
- $\mathbf{W}_{\mathbf{g$: hidden state에 대한 가중치 행렬, shape = ($d_h$, $d_h$) 
- $\mathbf{W}_{\mathbf{x}}$: hidden state에 대한 가중치 행렬, shape = ($d_h$, $d_h$) 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTQ5NTczOTYxLDEzMzU5MDY5NDgsLTEyNz
