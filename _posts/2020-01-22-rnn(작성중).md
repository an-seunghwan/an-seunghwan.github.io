...
*주의할 점은 RNN cell은 hidden state $\mathbf{h}$를 output으로 return하는 것이지, output sequence vector $\mathbf{y}$를 return하지 않는다는 것이다. output sequence vector $\mathbf{y}$는 별도의 출력층(output hidden layer)를 이용해서 return해야한다.*
...
Keras에서 RNN layer를 구현하기 위해서는 `layers.SimpleRNN`을 사용하면 된다. 주의할 점은 이 코드는 RNN layer만을 나타내는 것이고 output hidden layer에 대한 부분은 담겨있지 않다. 이 class의 arguments들의 의미를 하나씩 살펴보자. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxNDMxMTM0ODAsMTkxOTQ5MTgyNCwxNz
k5Njg2MDg0LDE0Njc4MjEzMjQsODg2NzQ2Nzc3LDE3NDU1Mzk5
MzQsNjkxMDY4MjI3LC0xMTk2ODczNDc2LDE2MjQ1MTc1MzgsLT
E4NjkyNjcyNzgsLTE0NTE0MTI1NjUsMTg3MzI5ODg3NywxODQ3
NjkyNjUyLC0xNDgxNTQwMjQyLDYwMTQwMTA2NiwxMzM1OTA2OT
Q4LC0xMjc5NDM5NzQxLC0xOTI3NjM3MTUyLC0xMDMwNDgzNTc0
XX0=
-->