...
`input_shape=(16, 64)`은 input sequence의 차원을 결정하는 argument이다. 이는 다르게 표현하면 `input_shape=(timesteps, input_dim)`이다. `timestep=16(=input_length)`는 기계번역의 경우에 input sequence의 길이, 즉 문장의 길이가 16임을 나타낸다. `input_dim(=embedding_size)`=$라고 할 수 있다.
...
그렇다면 `return_sequences`의 역할을 알아보자. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQzNTI3Mjc5MCwxNjI0NTE3NTM4LC0xOD
Y5MjY3Mjc4LC0xNDUxNDEyNTY1LDE4NzMyOTg4NzcsMTg0NzY5
MjY1MiwtMTQ4MTU0MDI0Miw2MDE0MDEwNjYsMTMzNTkwNjk0OC
wtMTI3OTQzOTc0MSwtMTkyNzYzNzE1MiwtMTAzMDQ4MzU3NF19
