...
- $d_{\mathbf{x}}$는 embedding 차원의 크기(embedding size)라고 할 수 있다.
...
Keras에서 RNN layer를 구현하기 위해서는 `layers.SimpleRNN`을 사용하면 된다. 이 class의 arguments들의 의미를 하나씩 살펴보자. 우선 `units=32`는 hidden state의 차원을 결정하는 argument로 $d_{\mathbf{h}}$와 동일하다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTA5OTk0ODg0OCwtMTg2OTI2NzI3OCwtMT
