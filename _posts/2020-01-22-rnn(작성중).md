...
일반적으로 $\mathbf{x}_t$는 기계번역 분야에서 하나의 단어(token)으로 간주된다. 따라서 $d_{\mathbf{x}}$는 embedding 차원의 크기(embedding size)라고 할 수 있다. 그리고 하나의 RNN layer는 가중치 $\mathbf{W}_\mathbf{x}, 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExNDU2OTQ4ODcsNjAxNDAxMDY2LDEzMz
