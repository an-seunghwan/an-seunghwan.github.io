...
$t$ 시점의 경우
- input: $\mathbf{x}_t$ (sequence data)
- hidden state: $\mathbf{h}_t$
- $\mathbf{W}_{\mathbf{x}}$: input sequence에 대한 가중치 행렬, shape = ($d_{\mathbf{x}}$, $d_{\mathbf{h}}$) 
- $\mathbf{W}_{\mathbf{h}}$: hidden state에 대한 가중치 행렬, shape = ($d_{\mathbf{h}}$, $d_{\mathbf{h}}$) 
- $\mathbf{W}_{\mathbf{y}}$: hidden state에 대한 가중치 행렬, shape = ($d_h$, $d_h$) 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwODc4MTMxODYsMTMzNTkwNjk0OCwtMT
