...
[그림]
| $\textbf{x}_t$ | t timestep input of sequence, shape = ($d_{\mathbf{x}}$, 1) |
|--|--|
| $\mathbf{h}_{t}$ | hidden state corresponding to $t$th timestep input, shape = ($d_{\mathbf{h}}$, 1) |
...
일반적으로 $\mathbf{x}_t$는 기계번역 분야에서 하나의 단어(token)으로 간주된다. 따라서 $d_{\mathbf{x}}$는 embedding 차원의 크기(embedding size)라고 할 수 있다. 그리고 하나의 RNN layer는 가중치 $\mathbf{W}_\mathbf{x}, \mathbf{W}_\mathbf{h}, \mathbf{W}_\mathbf{y}$를 모든 시점(timestep)에 대해서 동일하게 사용한다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNTgxNTIwNzAsLTE0ODE1NDAyNDIsNj
AxNDAxMDY2LDEzMzU5MDY5NDgsLTEyNzk0Mzk3NDEsLTE5Mjc2
MzcxNTIsLTEwMzA0ODM1NzRdfQ==
-->