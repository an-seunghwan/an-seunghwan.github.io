...
`input_shape=(16, 64)`은 input sequence의 차원을 결정하는 argument이다. 이는 다르게 표현하면 `input_shape=(timesteps, input_dim)`이다. `timesteps(or input_length)=16`는 기계번역의 경우에 input sequence의 길이, 즉 문장의 길이가 16임을 나타낸다. `input_dim(or embedding_size)=64`=$d_{\mathbf{x}}$라고 할 수 있습니다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTIwNzE3ODg2LC0xNjg5NTM5Mjc3LC0xNz
