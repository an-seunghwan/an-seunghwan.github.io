...
그렇다면 `return_sequences`의 역할을 알아보자. `return_sequences`는 각 RNN cell에서 계산되는 hidden state에 대한 return 여부를 나타낸다. 즉, 앞의 예제들은 `return_sequences=False`이므로 output으로 계산된 결과는 마지막 timestep에 해당하는 RNN cell이 return하는 hidden state에 해당합니다. `return_sequences=True`로 하게되면 모든 RNN cell에서 hidden state을 return하게 된다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjQyNTcxNTcxLC0xNjg5NTM5Mjc3LC0xNz
